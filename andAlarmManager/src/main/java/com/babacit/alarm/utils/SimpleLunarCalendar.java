...
public class SimpleLunarCalendar {
	...
	/** 十二生肖 */
	private final static String[] Animals = { "鼠", "牛", "虎", "兔", "龙", "蛇",
			"马", "羊", "猴", "鸡", "狗", "猪" };
	/** 农历中文字符串一 */
	private final static String[] lunarString1 = { "零", "一", "二", "三", "四",
			"五", "六", "七", "八", "九" };
	/** 农历中文字符串二 */
	private final static String[] lunarString2 = { "初", "十", "廿", "三", "正",
			"腊", "冬", "闰" };
	/** 农历年 */
	...
	private void init(long timeInMillis) {
		if (timeInMillis > minTimeInMillis && timeInMillis < maxTimeInMillis) {
			...
			this.lunarYear = 1900;
			int daysInLunarYear = SimpleLunarCalendar
					.getLunarYearDays(this.lunarYear);
			// 递减每个农历年的总天数，确定农历年份
			while (this.lunarYear < 2100 && offset >= daysInLunarYear) {
				offset -= daysInLunarYear;
				daysInLunarYear = SimpleLunarCalendar
						.getLunarYearDays(++this.lunarYear);
			}
			// 获取该农历年的闰月月份
			int leapMonth = SimpleLunarCalendar
					.getLunarLeapMonth(this.lunarYear);
			// 没有闰月则不是闰年
			...
			while (lunarMonth < 13 && offset > 0) {
				if (isLeap && !isDecrease) {
					// 该农历年闰月的总天数
					daysInLunarMonth = SimpleLunarCalendar
							.getLunarLeapDays(this.lunarYear);
					isDecrease = true;
				} else {
					// 该农历年正常农历月份的天数
					daysInLunarMonth = SimpleLunarCalendar.getLunarMonthDays(
							this.lunarYear, lunarMonth);
				}
			...
			this.maxDayInMonth = daysInLunarMonth != 0 ? daysInLunarMonth
					: SimpleLunarCalendar.getLunarMonthDays(this.lunarYear,
							lunarMonth);
	...
	private static int getLunarYearDays(int lunarYear) {
		...
		for (int i = 0x8000; i > 0x8; i >>= 1) {
			// 每个大月累加一天
			daysInLunarYear += ((SimpleLunarCalendar.lunarInfo[lunarYear - 1900] & i) != 0) ? 1
					: 0;
		}
		// 加上闰月天数
	...
	public static int getLunarMonthDays(int lunarYear, int lunarMonth) {
		...
		int daysInLunarMonth = ((SimpleLunarCalendar.lunarInfo[lunarYear - 1900] & (0x10000 >> lunarMonth)) != 0) ? 30
				: 29;
	...
	public String getMonthString() {
		...
		return (this.isLeap() ? "闰" : "")
				+ SimpleLunarCalendar.getLunarMonthString(this.lunarMonth);
	}
	...
	public String getDateString() {
		if (lunarYear == 0)
			return null;
		return this.getYearString() + "年" + this.getMonthString() + "月"
				+ this.getDayString() + "日";
	}
