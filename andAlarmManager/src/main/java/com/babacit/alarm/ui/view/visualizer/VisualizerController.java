...
public class VisualizerController {
	...
	@SuppressLint("NewApi")
	public void linkAudio(AudioRecord record) {
		if (record == null) {
			throw new NullPointerException("Can not link to null AudioRecord");
		}

		mVisualizer = new Visualizer(record.getAudioSessionId());
		Equalizer mEqualizer = new Equalizer(0, record.getAudioSessionId());
		mEqualizer.setEnabled(true);
		mVisualizer.setCaptureSize(Visualizer.getCaptureSizeRange()[1]);
...
//		});
	...
	public void colorUpdate(int red, int green, int blue) {
		for (VisualizerView v : views) {
			v.setColour(Color.argb(255, red, green, blue));
		}
	}
